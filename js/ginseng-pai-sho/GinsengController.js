/* Ginseng Pai Sho specific UI interaction logic */

function GinsengController(gameContainer, isMobile) {
	/* Set default preferences */
	if (!localStorage.getItem(GinsengTileDesignTypeKey)
			|| !GinsengController.tileDesignTypeValues[localStorage.getItem(GinsengTileDesignTypeKey)]) {
		localStorage.setItem(GinsengTileDesignTypeKey, "tggGinseng");
	}

	this.actuator = new GinsengActuator(gameContainer, isMobile, this.isAnimationsEnabled());
...
GinsengController.animationsEnabledKey = "AnimationsEnabled";

GinsengController.prototype.getGameTypeId = function() {
	return GameType.GinsengPaiSho.id;
};
...
GinsengController.prototype.completeSetup = function() {
	...
	if (gameOptionEnabled(SWAP_BISON_WITH_LEMUR)) {
		GinsengController.setTileDesignsPreference("tggGinseng");
	}
};
...
GinsengController.prototype.resetNotationBuilder = function(applyDrawOffer) {
	this.notationBuilder = new GinsengNotationBuilder();
	if (applyDrawOffer) {
...
GinsengController.prototype.getNewGameNotation = function() {
	return new GinsengGameNotation();
};

GinsengController.getHostTilesContainerDivs = function() {
	if (gameOptionEnabled(SWAP_BISON_WITH_LEMUR)) {
		return '<div class="HC"></div> <div class="H' + GinsengTileCodes.FlyingLemur + '"></div> <div class="HB"></div> <div class="HW"></div> <br class="clear" /> <div class="HF"></div> <div class="HD"></div> <div class="H_empty"></div> <div class="HL"></div>';
	}
...
GinsengController.getGuestTilesContainerDivs = function() {
	if (gameOptionEnabled(SWAP_BISON_WITH_LEMUR)) {
		return '<div class="GC"></div> <div class="G' + GinsengTileCodes.FlyingLemur + '"></div> <div class="GB"></div> <div class="GW"></div> <br class="clear" /> <div class="GF"></div> <div class="GD"></div> <div class="G_empty"></div> <div class="GL"></div>';
	}
...
GinsengController.prototype.resetMove = function() {
...
GinsengController.prototype.getDefaultHelpMessageText = function() {
	var helpText = "<h4>Ginseng Pai Sho</h4> <p> <p>Ginseng Pai Sho is the Pai Sho variant seen in the fanfiction story <a href='https://skudpaisho.com/site/more/fanfiction-recommendations/' target='_blank'>Gambler and Ginseng (download here)</a>.</p> <p><strong>You win</strong> if you capture your opponent's White Lotus tile.</p> <p><strong>On a turn</strong>, you may either deploy a tile or move a tile.</p> <p><strong>You can't capture Flower tiles</strong> until your White Lotus has been deployed.<br /> <strong>You can't capture Non-Flower tiles</strong> until both players' White Lotus tiles have been deployed.</p> <p><strong>Hover</strong> over any tile to see how it works.</p> </p> <p>Select tiles to learn more or <a href='https://skudpaisho.com/site/games/Ginseng-pai-sho/' target='_blank'>view the rules</a>.</p>";
	return helpText;
};

GinsengController.prototype.togglePeekAtOpponentMoves = function() {
...
GinsengController.prototype.getAdditionalMessage = function() {
	...
	if (this.gameNotation.moves.length === 0) {
		...
		msg += getGameOptionsMessageHtml(GameType.GinsengPaiSho.gameOptions);
		...
		if (boardPoint.isType(POSSIBLE_MOVE) && myTurn()) {
			...
			if (!this.checkingOutOpponentTileOrNotMyTurn && !isInReplay) {
				...
				if (onlinePlayEnabled && this.gameNotation.moves.length === 1) {
					createGameIfThatIsOk(GameType.GinsengPaiSho.id);
				} else {
					if (playingOnlineGame()) {
						callSubmitMove();
					} else {
...
GinsengController.prototype.getTheMessage = function(tile, ownerName) {
	...
	var heading = GinsengTile.getTileName(tileCode);
...
GinsengController.prototype.getTileMessage = function(tileDiv) {
	...
	var tile = new GinsengTile(divName.substring(1), divName.charAt(0));
...
GinsengController.prototype.cleanup = function() {
	// document.querySelector(".svgContainer").classList.remove("GinsengBoardRotate");
};
...
GinsengController.prototype.getAdditionalHelpTabDiv = function() {
	...
	heading.innerText = "Ginseng Pai Sho Preferences:";
	...
	settingsDiv.appendChild(GinsengController.buildTileDesignDropdownDiv());
...
GinsengController.prototype.buildToggleAnimationsDiv = function() {
...
GinsengController.prototype.toggleAnimations = function() {
	if (this.isAnimationsEnabled()) {
		setUserGamePreference(GinsengController.animationsEnabledKey, "false");
		this.actuator.setAnimationOn(false);
	} else {
		setUserGamePreference(GinsengController.animationsEnabledKey, "true");
		this.actuator.setAnimationOn(true);
	}
	clearMessage();
};

GinsengController.prototype.isAnimationsEnabled = function() {
	/* Check !== false to default to on */
	return getUserGamePreference(GinsengController.animationsEnabledKey) !== "false";
};

/* Ginseng tile designs */
GinsengController.tileDesignTypeValues = {
	tggGinseng: "The Garden Gate Designs",
...
GinsengController.setTileDesignsPreference = function(tileDesignKey) {
	localStorage.setItem(GinsengTileDesignTypeKey, tileDesignKey);
	if (gameController && gameController.callActuate) {
		gameController.callActuate();
	}
};

GinsengController.buildTileDesignDropdownDiv = function(alternateLabelText) {
	var labelText = alternateLabelText ? alternateLabelText : "Tile Designs";
	return buildDropdownDiv("GinsengPaiShoTileDesignDropdown", labelText + ":", GinsengController.tileDesignTypeValues,
							localStorage.getItem(GinsengTileDesignTypeKey),
							function() {
								GinsengController.setTileDesignsPreference(this.value);
							});
};

GinsengController.prototype.setAnimationsOn = function(isAnimationsOn) {
